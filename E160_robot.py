...
class E160_robot:
    ...
    def __init__(self, environment, address, robot_id):
        ...
        self.file_name = 'Log/'+str(CONFIG_COURSE) +'_Noise_Testing_' + str(CONFIG_SENSOR_NOISE_SIM)[:4] + '_' + str(CONFIG_NUM_SENSORS)+ '_Sensors_' + datetime.datetime.now().replace(microsecond=0).strftime('%y-%m-%d %H.%M.%S') + '.txt'
